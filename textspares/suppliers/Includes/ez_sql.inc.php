<?
	...
	class db {
		...
		function query($query, $output = OBJECT, $report = false) 
		{
			global $settings;
			...
			if ( mysql_error() ) 
			{
				$replace = array('FROM','WHERE');
				$with = array('<br/>FROM','<br/>WHERE');
				if(!$report) $report = debug_backtrace(false);

				$error_message = '<div class="error_frame"><div class="mysql_error"><b>MYSQL ERROR</b><br/>'.mysql_error();
				if(count($report) > 1)
				{
					$error_message .= '<br/><br/><strong>Trace Source of Error:</strong><br/><br/>';
				}
				
				foreach($report AS $hunt)
				{
					$error_message .= '<br/><strong>'.$hunt['file'].'</strong>; On Line: <strong>'.$hunt['line'].'</strong><br/>';
				}
				
				$error_message .= '<br/><br/><font>'.str_replace($replace,$with,$query).'</font></div></div>';
				if($settings['debug_mode'])
				{
					echo $error_message;
				}
				@send_email('webmaster@textspares.co.uk','reports@textspares.co.uk','New Error Report'.date('D j M Y - h:i A'),$error_message,false);
				return false;
			}
			else
			{
