...
function sales($db)
{
	...
	<h1><img style="vertical-align: middle;" alt="" width="35" height="35" src="<?php echo ROOT; ?>Images/quotes.png">My Sales</h1>
	...
	$(document).ready(function()
	{
		...
		$(".proccess_order").click(function(e)
		{
			e.preventDefault();
			$data = "#"+$(this).attr('id');
			$qid = $data.split("-");
			if($qid[0] == '#payment')
			{
				$p = confirm('You are about to confirm successful payment for the order.\nThe customer will be notified and their order status changed to "Awaiting Dispatch"\n\nAre you sure you wish to continue?');
				if($p)
				{
					$url = "<?php echo ROOT; ?>?_action=sales&_ajax=true&_subaction=order_payment";
					$.ajaxSetup({cache: false});
					$.get(
						$url,
						{ _qid: $qid[1] },
						function(data) {
							...
							if($result == "Success")
							{
								$($data).html('<span class="ui-icon ui-icon-arrowstop-1-s"></span>&nbsp;Dispatched');
								$($data).attr('id','dispatch-'+$qid[1]);
								$('#'+$qid[1]).removeClass('quote_blue zebra1 zebra2').addClass('quote_orange').children('#ordered').text('Waiting Dispatch');
								$('#cancel-'+$qid[1]).remove();
								$('#delivery-dispatch-'+$qid[1]).css('display','inline-block');
								$('#after-payment-'+$qid[1]).css('display','inline-block');
							} else {
								return alert($result);
							}
						},
			...
			} else if($qid[0] == '#dispatch')
			{
				$p = confirm('Notify the customer their order has been dispatched?');
				if($p)
				{
					$courier = $('#courier-'+$qid[1]).val();
					$parcelid = $('#parcelid-'+$qid[1]).val();
					$url = "<?=ROOT;?>?_action=sales&_ajax=true&_subaction=order_dispatch";
					$.ajaxSetup({cache: false});
					$.get(
						$url,
						{ _qid: $qid[1], _courier: $courier, _parcelid: $parcelid },
						function(data) {
							$ele = '<div id="'+'result">';
							$start_ele = data.indexOf($ele);
							$start_pos = $start_ele+17;
							...
							$result = data.substr($start_pos,$length);
							if($result == "Success") {
								$('#'+$qid[1]).removeClass('quote_blue zebra1 zebra2 quote_orange').addClass('quote_green').children('#ordered').text('Complete');
								$('#buttons-'+$qid[1]).html('<span class=\"_saved\"><span class=\"ui-icon ui-icon-check\"></span>Order Completed</span>');
							} else {
								return alert($result);
							}
						},
		...
		$(".cancel_order").click(function(e)
		{
			...
			if($p) {
				...
				$url = "<?php echo ROOT; ?>?_action=sales&_ajax=true&_subaction=cancel_order";
				$.ajaxSetup({cache: false});
				$.get(
					$url,
					{ _qid: $qid[1] },
					function(data) {
						...
						if($result == "Success") {
							...
							$('#buttons-'+$qid[1]).html('<span class=\"_saved\"><span class=\"ui-icon ui-icon-check\"></span>Order Cancelled</span>');
						} else {
							return alert($result);
						}
					},
	...
	$sql = 'SELECT
		...
		WHERE `sq`.`company_user_id` = '.$session->userdata['id'].$get_record.' AND `sq`.`accepted` > 0 AND `sq`.`dispatched` = 0
    ...
    if($quotes)
	{
		...
		<br clear="all" />
		<div id="list_options">All Orders <?php echo (($session->userdata['vat'] > 0) ? 'include' : 'exclude'); ?> V.A.T.</div>
		<br clear="all" />
		...
		{
			$status = ($quotes->transaction_id > 0) ? (($quotes->dispatched > 0) ? 'Completed' : 'Waiting Dispatch') : 'Waiting Payment';
		...
		?>
			<table>
				<tr class="<?=(($quotes->transaction_id > 0) ? (($quotes->dispatched == 0) ? 'quote_orange ' : 'quote_green') : zebra($i));?> rowover" id="<?=$quotes->quote_id;?>" onclick="show_details(this.id);">
					<td width="80"><?php echo pad_ref_number($quotes->vehicle_id).' - '.$session->userdata['id']; ?></td>
			...
			<div style="display:<?=(($display_sale == $quotes->quote_id) ? 'block' : 'none');?>" class="details" id="DT<?php echo $quotes->quote_id; ?>">
				<div class="m10 left w30">
					...
					<div class="reg_grp ui-corner-all" style="margin-top:20px;">
						...
						<table class="newuser_tbl">
							<tr>
								<td>
								<?='<div id="after-payment-'.$quotes->quote_id.'" style="display:'.(($quotes->transaction_id > 0) ? 'block' : 'none').';"><button id="invoice-'.$quotes->quote_id.'" class="print_invoice">Print Invoice</button> &nbsp; <!--//<button id="refund-'.$quotes->quote_id.'" class="refund_order">Proccess Refund</button> //--></div>';?>
								</td>
							</tr>
						</table> 
					</div>
				...
				echo '
						...
						<tr>
							...
							<td style="background-color:#FFF;"><b>'.(($session->userdata['vat'] > 0) ? '&pound;'.number_format(($total*($settings['vat_rate']/100))+$total,2) : 'N/A').'</b></td>
						</tr>
						<tr>
							<td colspan="4" style="text-align:right;padding:10px;" id="buttons-'.$quotes->quote_id.'">
				';
				if($quotes->dispatched == 0) {
					echo '
								<div id="delivery-dispatch-'.$quotes->quote_id.'" class="delivery-dispatch" style="display:'.(($quotes->transaction_id > 0) ? 'inline-block' : 'none').';">
								Courier: <input type="text" id="courier-'.$quotes->quote_id.'" name="courier-'.$quotes->quote_id.'" value="DHL" /> &nbsp; 
								Tracking ID: <input type="text" id="parcelid-'.$quotes->quote_id.'" name="parcelid-'.$quotes->quote_id.'" value="" />
								</div>
								<button id="'.(($quotes->transaction_id > 0) ? 'dispatch' : 'payment').'-'.$quotes->quote_id.'" class="proccess_order">'.(($quotes->transaction_id > 0) ? 'Order Dispatched' : 'Payment Processed').'</button>
								'.(($quotes->transaction_id == 0) ? '<button id="cancel-'.$quotes->quote_id.'" class="cancel_order">Cancel Order</button>' : '').'
					';
				} else {
					echo '<div style="float:left;display:inline;"><b>Courier:</b> '.$quotes->courier.' &nbsp; <b>Tracking ID:</b> '.$quotes->tracking_id.'</div><span class=\"_saved\"><span class=\"ui-icon ui-icon-check\"></span>Order Completed</span>';
				}
				echo '
							</td>
						</tr>
		...
		?>
         <div class="ui-widget m10">
                <div class="ui-state-highlight ui-corner-all" style="padding: 0pt 0.7em;"> 
                	 <p><span class="ui-icon ui-icon-info"  style="position:relative;float:left;margin-top:0.1em;margin-right:1em;"></span>There are no active sales to be proccessed. <a href="?_action=history">View Sales History</a></p>
                </div>
            </div>
       <?php
	} 
			...
			if(is_numeric($qid) && $qid > 0)
			{
				...
				if(mysql_affected_rows() > 0)
				{
					...
					if(mysql_affected_rows() > 0)
					{
						...
						$sql = 'INSERT INTO `customer_transactions`(`record_time`,`customer_id`,`quotation_id`,`company_id`,`company_user_id`,`payment_method`,`amount`,`vat_rate`) VALUES(\''.time().'\',\''.$info->customer_id.'\',\''.$qid.'\',\''.$session->userdata['companyid'].'\',\''.$session->userdata['id'].'\',\'PHONE_ORDER\',\''.$total.'\',\''.$vat.'\')';
									...
									if(mysql_affected_rows() > 0)
									{
										...
										if(mysql_query($sql)) {
											...
											$message = '
												...
												<p>You can login and contact the supplier if there are any problems: <a href="http://www.textspares.co.uk/?k='.create_emailloginkey($info->customer_id).'">Login</a>.</p>
		...
		if($uitest == true) {
			...
			if(is_numeric($qid) && $qid > 0)
			{
				...
				if(mysql_affected_rows() > 0)
				{
					...
					if($db->query($sql))
					{
						$sql = 'UPDATE `customer_requests` SET `compleated` = \''.time().'\' WHERE `order_group` = \''.$qid.'\'';
						if($db->query($sql))
						{
							echo '<div id="result">Success</div>';
							$message = '
							...
							$sql = 'UPDATE `supplier_quotes` SET `dispatched` = 0 WHERE `quote_id` = \''.$qid.'\'';
							$db->query($sql);
							echo '<div id="result">There was a problem setting the status to Compleated.</div>';
						}
					} else {
						echo '<div id="result">There was a problem setting the status to Order Dispatched.</div>';
					}
				} else {
					echo '<div id="result">No Matching Records found!</div>';
				}
			} else {
				echo '<div id="result">ERROR: Invalid Quotation ID sent to server.</div>';
			}
		}
