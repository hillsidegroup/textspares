<?
    //echo"1herere--->".$_POST[cname];
    
    $_POST = tep_clean_get__recursive($_POST);
    
    
  $HTTP_GET_VARS = tep_clean_get__recursive($HTTP_GET_VARS);
  
  //echo"2herere--->".$_POST[cname];

  $_GET = tep_clean_get__recursive($_GET);
  
  $_REQUEST = $_GET + $_POST;
  
  fwr_clean_global($_GET);
  
  function tep_clean_get__recursive($get_var)
  {
		  if (!is_array($get_var))
		  {
		  	 $get_var =  preg_replace("/[^ @{}a-zA-Z0-9_.-]/i", "", urldecode($get_var));
		  	 //$get_var =  RemoveXSS($get_var);
