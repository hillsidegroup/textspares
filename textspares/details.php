...
$res_file['pageTitle'] = 'New and Used'.($meta_make ? ' '.($meta_make) : '').' Car Spares '.($meta_area ? $meta_area : 'UK');
top();
?>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr><td >
        	 <table cellpadding="0" cellspacing="0" width="100%" border="0">
        	 	<tr><td><a href="<?=BASE?>index.php" class='toplinks'>Home</a> >
        	 	 <a href="<?=BASE?>Details/<?=$_REQUEST['fileid']?>/<?=$CNAME?>" class='toplinks'><?=$mkname?></a></td></tr>
        		<tr>
              <td valign="top">
              	<table width="100%" border="0" align="center" cellpadding="1" cellspacing="0">
              		<tr><td>
              		<?php
              		$sqld=mysql_query('SELECT * FROM `files` WHERE `file_id` = \''.$db->mysql_prep($_REQUEST['fileid']).'\'')or die(mysql_error());
